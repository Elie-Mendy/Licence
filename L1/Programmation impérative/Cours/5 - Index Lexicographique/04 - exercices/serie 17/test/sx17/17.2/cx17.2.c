...
void indexe( char * ligne, idx ref){
  ...
  while (mot){
    ...
    int s = indice(mot, stop);                      // verificaton de la présence du mot dans l'index
    if (s < 0){                                     // si le mot n'est pas a exclure
      int x = indice(mot, mots);                      // verificaton de la présence du mot dans l'index
...
int indice(str mot, char * table[]){
  idx i = 0;
  for (i = 0; table[i]; i++){
    if (pareil(mot, table[i])) return i;
  }
  return -1;
}
